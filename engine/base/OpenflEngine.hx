...
class OpenflEngine
{
	...
	public static var justLeftMouse:Bool = false;
	...
	public static var playSound:String->?String->?SoundConfig->Void = OpenflSm.playSound;
	public static var stopChannel:String->Void = OpenflSm.stopChannel;
	public static var resetGlobalVolume:Void->Void = OpenflSm.resetGlobalVolume;
	public static var setChannelVolume:String->Float->Void = OpenflSm.setChannelVolume;
	public static var fadeChannel:String->?Bool->?Float->Void = OpenflSm.fadeChannel;
	...
	public static function init(gameInit:Void->Void, updateLoop:Void->Void):Void {
		...
		cameraViewport.setTo(0, 0, width, height);
		...
		stage.addEventListener(MouseEvent.MOUSE_DOWN, function(_){justLeftMouse=leftMouse=true;});
		...
		stage.addEventListener(MouseEvent.MOUSE_WHEEL, function(e:MouseEvent){mouseWheel=e.delta; e.preventDefault();});
	...
	public static function drawTextNew(id:Int):Void {
		...
		function setFormatParams(region:FormatRegion=null):Void {
			var fontName:String;
			if (region == null) {
				fontName = bundle.textFieldFormat.fontName+"-Regular";
				fontSize = bundle.textFieldFormat.fontSize;
			} else {
				if (region.italic) fontName = bundle.textFieldFormat.fontName+"-Italic";
				else if (region.bold) fontName = bundle.textFieldFormat.fontName+"-Bold";
				else fontName = bundle.textFieldFormat.fontName+"-Regular";
			...
			var fontOptions:Array<BitmapFont> = [];
			for (f in fonts) {
				var name:String = f.page[2].split("/").last().split("_")[0];
				if (name == fontName) fontOptions.push(f);
			}
			font = fontOptions[0];
	...
	public static function drawTextOld(id:Int):Void {
		...
		for (region in bundle.regions) {
			...
			textFormat.align = defaultFormat.center ? "center" : "left";
	...
	public static function getTextCharRect(id:Int, charIndex:Int):Rectangle {
	...
	public static function transformSprite(id:Int, x:Float, y:Float, scaleX:Float, scaleY:Float, rotation:Float, alpha:Float, visible:Bool, centerPivot:Bool, layer:Int, pivotX:Float, pivotY:Float, scrolls:Bool):Void {
		var bundle:SpriteBundle = spriteBundles[id];
		var origWidth:Float;
		...
		if (bundle.magicSprite != null) bundle.magicSprite.transform.matrix = m;
		bundle.bitmap.transform.matrix = m;
		bundle.trans = m.clone();
		bundle.scrolls = scrolls;
		bundle.bitmap.alpha = alpha;
		bundle.bitmap.visible = visible;
	}
	...
	public static function tintSprite(id:Int, colour:Int):Void {
		...
		if (bundle.curFrameRect != null) {
			switchFrame(
				id,
				Std.int(bundle.curFrameRect.x), Std.int(bundle.curFrameRect.y), Std.int(bundle.curFrameRect.width), Std.int(bundle.curFrameRect.height),
				Std.int(bundle.curFrameOffset.x), Std.int(bundle.curFrameOffset.y)
			);
		} else {
	...
	public static function update(e:Event):Void {
		mouse.x = stage.mouseX * (cameraViewport.width / stage.stageWidth);
		mouse.y = stage.mouseY * (cameraViewport.height / stage.stageHeight);
...

		{
			...
			// function qsortLayerIndex(a, b):Int {
			// 	var spr1:SpriteBundle = a;
			// 	var spr2:SpriteBundle = b;

			// 	if (spr1.layer < spr2.layer) return -1;
			// 	if (spr1.layer > spr2.layer) return 1;

			// 	var id1:Int = spriteBundles.indexOf(spr1);
			// 	var id2:Int = spriteBundles.indexOf(spr2);
			// 	if (id1 < id2) return -1;
			// 	if (id1 > id2) return 1;
			...
			// layered.sort(qsortLayerIndex);
		}

		var camScaleX:Float = stage.stageWidth/cameraViewport.width;
		var camScaleY:Float = stage.stageHeight/cameraViewport.height;
		var camPosX:Float = -cameraViewport.x * camScaleX;
		var camPosY:Float = -cameraViewport.y * camScaleY;

		for (bundle in spriteBundles) {
			if (bundle == null || bundle.bitmap == null) continue;

			m.copyFrom(bundle.trans);
			m.scale(camScaleX, camScaleX);
			if (bundle.scrolls) m.translate(camPosX, camPosY);
	...
	public static function pointOverlapSprite(p:Point, id:Int, pixelPerfect:Bool=false):Bool {
		...
		var pointInRect:Bool = bitmap.hitTestPoint(p.x, p.y, false);
	...
	public static function createSprite():Int {
		...
		gameSprite.addChild(bundle.bitmap);
		spriteBundles.push(bundle);
		return spriteBundles.length-1;
	}

	public static function newBitmapData():BitmapData {
		log(bitmapDataPool.length+" left in pool");
		if (bitmapDataPool.length == 0) bitmapDataPool.push(new BitmapData(width, height));

		var data:BitmapData = bitmapDataPool.pop();
		rect.setTo(0, 0, width, height);
	...
	public inline static function resolveAssetName(assetId:String, crash:Bool=true, extraContains:String=".png"):String {
		var realId:String = "";
	...
	public static function getBitmapData(assetId:String):BitmapData {
		assetId = resolveAssetName(assetId, true, ".png");
		if (extendedAssetNames.contains(assetId))
			return extendedAssetDatas[extendedAssetNames.indexOf(assetId)];
	...
	public static function switchFrame(id:Int, xpos:Int, ypos:Int, w:Int, h:Int, xoff:Int, yoff:Int):Void {
		var bundle:SpriteBundle = spriteBundles[id];
		if (bundle.curFrameRect == null) bundle.curFrameRect = new Rect();
		if (bundle.curFrameOffset == null) bundle.curFrameOffset = new Point();
		bundle.curFrameRect.setTo(xpos, ypos, w, h);
		bundle.curFrameOffset.setTo(xoff, yoff);
	...
	public static function setup9SliceSprite(id:Int, assetId:String, x1:Float, y1:Float, x2:Float, y2:Float, w:Float, h:Float):Void {
		...
		for (cx in 0...3) {
			for (cy in 0...3) {
				...
				// trace("Params from ("+w+","+h+"): "+"|"+origin.x+"|"+origin.y+"|"+origin.width+"|"+origin.height+"|"+draw.x+"|"+draw.y+"|"+draw.width+"|"+draw.height);
			}
		}
	...
	public static function addSprite(id:Int):Void {
	}
	...
	public static function applyGlowEffect(id:Int, colour:Int, alpha:Float, blurX:Float, blurY:Float, strength:Float, inner:Bool, knockout:Bool):Void {
		if (spriteBundles[id] == null) return;
	...
	public static function clearEffects(id:Int):Void {
		...
		if (spriteBundles[id].glitchEffectTimerId != -1) {
			timerCancel(spriteBundles[id].glitchEffectTimerId);
			spriteBundles[id].glitchEffectTimerId = -1;
	...
	private static function renderToData(src:BitmapData, dest:BitmapData, sx:Int, sy:Int, sw:Int, sh:Int, dx:Int, dy:Int, tint:Int=-1, scaleX:Float=1, scaleY:Float=1):Void {
		// trace("renderToData: "+sx+" "+sy+" "+sw+" "+sh+" "+dx+" "+dy+" "+tint+" "+scaleX+" "+scaleY);
	...
	public static function applyGlitchEffect(id:Int):Void {
		...
		var timerId:Int;
		timerId = spriteBundles[id].glitchEffectTimerId = timer(2, function() {
	...
	public static function destroySprite(id:Int):Void {
		...
		if (gameSprite.contains(bundle.bitmap)) gameSprite.removeChild(bundle.bitmap);
		if (bundle.mask != null) bundle.mask.bitmapData.dispose();
		if (bundle.magicSprite != null) stage.removeChild(bundle.magicSprite);
	...
	public static function loadFromDisk(callback:String->String->Void):Void {
		...
		try {
			...
			callback(e.message, "error #"+e.errorID);
		}
#elseif js
		// @incomplete Mobile
		var fileName:String = "";
	...
	public inline static function getAssetList():Array<String> {
...
class SpriteBundle {
	...
	public var curFrameRect:Rect = null;
	public var curFrameOffset:Point = null;
	public var origSize:Point = new Point();
	public var magicSprite:Sprite = null;
	public var layer:Int = 0;
	public var tint:Int = 0;

	public var glitchEffectTimerId:Int = -1;
